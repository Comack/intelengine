...
function formatForensicsLine(data: StoryData): string {
  if (!data.forensics || data.forensics.flagged <= 0) return '';
  const nearLive = data.forensics.nearLive > 0 ? `, ${data.forensics.nearLive} near-live` : '';
  const minPValue = typeof data.forensics.minPValue === 'number' && Number.isFinite(data.forensics.minPValue)
    ? `, p≤${data.forensics.minPValue.toFixed(3)}`
    : '';
  const suffix = data.forensics.flagged === 1 ? 'y' : 'ies';
  return `🧪 Forensics: ${data.forensics.flagged} anomal${suffix}${nearLive}${minPValue}\n`;
}
...
export const shareTexts = {
  ...
  linkedin: (data: StoryData) =>
    ...
    `${data.forensics?.flagged > 0 ? `• Forensics anomalies: ${data.forensics.flagged}${data.forensics.nearLive > 0 ? ` (${data.forensics.nearLive} near-live)` : ''}\n` : ''}` +
