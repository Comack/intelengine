...
│  Proto gateway (18 typed services)  │
...
- **Vercel CLI** — `npm i -g vercel` (required to run edge functions locally)
- **Git**

Optional for desktop builds: Rust toolchain + OS build prerequisites (see [Desktop Build](#desktop-app-tauri)).
...
```bash
vercel dev       # Frontend + all API edge functions on http://localhost:3000
```

> **Why `vercel dev` and not `npm run dev`?**
> `vercel dev` emulates the full edge runtime locally — all `api/` endpoints work, news feeds load, AI summaries run.
> `npm run dev` starts only the Vite frontend; news, markets, and AI panels will show "No data".

### 3. Choose a Variant (Optional)

The same codebase produces three specialized dashboards:

```bash
vercel dev                        # Full geopolitical variant (default)
VITE_VARIANT=tech vercel dev      # Tech Monitor variant
VITE_VARIANT=finance vercel dev   # Finance Monitor variant
```

Or use the npm shortcuts:

```bash
npm run dev              # Full variant, Vite only (no edge functions)
npm run dev:tech         # Tech variant, Vite only
npm run dev:finance      # Finance variant, Vite only
```

### 4. What Works Without API Keys

The core dashboard is fully functional with zero configuration:

| Feature | Without keys | With keys |
|---------|-------------|-----------|
| Interactive 3D globe | ✅ Always | ✅ |
| 30+ static layers (bases, cables, pipelines, minerals) | ✅ Always | ✅ |
| Earthquakes (USGS) | ✅ Always | ✅ |
| Severe weather alerts (NWS) | ✅ Always | ✅ |
| Natural disasters (NASA EONET, GDACS) | ✅ Always | ✅ |
| News feeds (100+ RSS sources) | ✅ Always | ✅ |
| Prediction markets (Polymarket) | ✅ Always | ✅ |
| Space weather (NOAA SWPC) | ✅ Always | ✅ |
| Radiation (Safecast) | ✅ Always | ✅ |
| Tsunami warnings (NWS) | ✅ Always | ✅ |
| Social trends (Bluesky) | ✅ Always | ✅ |
| Deforestation alerts (GFW) | ✅ Always | ✅ |
| Pollution layer (Sentinel-5P) | ✅ Always | ✅ |
| AI summaries | ❌ | ✅ Groq / OpenRouter / Ollama |
| Stock & market data | ❌ | ✅ Finnhub |
| Economic indicators | ❌ | ✅ FRED, EIA |
| Internet outages map | ❌ | ✅ Cloudflare Radar |
| Protest data | ❌ | ✅ ACLED |
| Live AIS vessel tracking | ❌ | ✅ AISStream relay |
| Live military flights | ❌ | ✅ OpenSky relay |
| Satellite fire detection | ❌ | ✅ NASA FIRMS |
| Aircraft enrichment | ❌ | ✅ Wingbits |
| Port congestion | ❌ | ✅ Portcast |
| Dark ship detection | ❌ | ✅ Global Fishing Watch |
| Conflict incidents | ❌ | ✅ Liveuamap |
| Power grid data | ❌ | ✅ Electricity Maps |
...
Key groups (all optional — omit any and the corresponding layer is simply hidden):
...
World Monitor's API layer runs as **Vercel Edge Functions** (`api/[domain]/v1/[[...path]].ts`). Running `npm run dev` alone starts only the Vite frontend — edge functions won't execute and most panels (news feeds, markets, AI summaries) will be empty.

### Option 1: Deploy to Vercel (Recommended)

Vercel runs all edge functions natively on their free Hobby tier. Takes ~5 minutes:
...
Then add your API keys in **Vercel Dashboard → Your Project → Settings → Environment Variables**.

**Minimum keys for a useful deployment:**

| Variable | Service | Get it |
|----------|---------|--------|
| `GROQ_API_KEY` | AI summaries | [console.groq.com](https://console.groq.com/) — free |
| `FINNHUB_API_KEY` | Stock market data | [finnhub.io](https://finnhub.io/) — free |
| `FRED_API_KEY` | Economic indicators | [fred.stlouisfed.org/docs/api](https://fred.stlouisfed.org/docs/api/api_key.html) — free |
| `CLOUDFLARE_API_TOKEN` | Internet outage map | [dash.cloudflare.com → Profile → API Tokens](https://dash.cloudflare.com/profile/api-tokens) — free |
| `ACLED_ACCESS_TOKEN` | Protest & conflict data | [acleddata.com/register](https://acleddata.com/register/) — free for research |
| `UPSTASH_REDIS_REST_URL` + `TOKEN` | AI deduplication + caching | [console.upstash.com](https://console.upstash.com/) — free 10K cmd/day |
...
#### Deploying Multiple Variants

Each variant is a separate Vercel project with `VITE_VARIANT` set:

```bash
# World Monitor (full geopolitical)
vercel --env VITE_VARIANT=full

# Tech Monitor
vercel --env VITE_VARIANT=tech

# Finance Monitor
vercel --env VITE_VARIANT=finance
```

Or set `VITE_VARIANT` in the Vercel dashboard per-project and connect each to the same repo.

### Option 2: Local Development with Vercel CLI

To run everything locally (frontend + edge functions + all `api/` routes):
...
> **Use `vercel dev` not `npm run dev`** — the Vercel CLI emulates the edge runtime so all `api/` endpoints work. Plain `npm run dev` starts only Vite.

To run a specific variant locally:

```bash
VITE_VARIANT=tech vercel dev
VITE_VARIANT=finance vercel dev
```

### Option 3: Static Frontend Only

For the map and client-side features without any backend (no news feeds, no AI, no market data):
...
The interactive 3D globe, static data layers (bases, cables, pipelines, minerals), Safecast radiation, USGS earthquakes, NWS weather/tsunami, and browser-side ML models all work without the API layer.

### Option 4: Desktop App (Tauri)

The desktop app bundles a local API sidecar so it works fully offline without Vercel:

```bash
npm install
npm run desktop:dev          # Dev mode (full variant)
```

**Prerequisites**: Rust toolchain + OS build tools (Xcode CLI on macOS, Visual Studio Build Tools on Windows). See [`docs/RELEASE_PACKAGING.md`](./docs/RELEASE_PACKAGING.md) for full build and packaging instructions for all three variants.

The desktop app stores API keys in the OS keychain (macOS Keychain, Windows Credential Manager, Linux Secret Service) — no plaintext key files. Keys are entered through the in-app Settings panel.

### Redis / Upstash Setup (Optional but Recommended)

Redis enables cross-user AI response caching (deduplication) and pre-computed risk score caching. Without it, AI summaries are generated per-request and risk score panels may be slower.
...
The 10K commands/day free tier is sufficient for a personal or small-team deployment. For high-traffic instances, upgrade to the Pay-as-you-go tier.
...
| **Linux x86_64**       | ✅ Full support          | `vercel dev` works. Desktop `.AppImage` available for x86_64                                                   |
| **macOS (Apple/Intel)**| ✅ Full support          | Both `vercel dev` and desktop `.dmg` work                                                                      |
| **Windows**            | ✅ Full support          | `vercel dev` works. Desktop `.exe`/`.msi` available                                                            |
| **Raspberry Pi / ARM** | ⚠️ Partial              | `vercel dev` edge runtime may not work on ARM. Use Vercel cloud deploy + `npm run dev` for the frontend        |
