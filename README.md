...
- **Forensics Intelligence Engine** — a multi-phase shadow analytics pipeline that continuously fuses 30+ real-time signal sources into calibrated anomaly scores and causal graphs; see the dedicated section below

### Forensics Intelligence Engine

A server-side shadow analytics pipeline (`server/worldmonitor/intelligence/v1/forensics-orchestrator.ts`) that runs continuously in the background, fusing signals from 30+ data sources into calibrated anomaly scores and causal relationships, surfaced in the **ForensicsPanel** UI.

**Pipeline phases:**
- **Signal Normalization** — validates and normalizes incoming `ForensicsSignalInput[]` records (sourceId, region, domain, signalType, value, confidence, observedAt, evidenceIds) before any fusion step
- **Weak Supervision EM Fusion** — Expectation-Maximization algorithm with per-signal-type learned weights, Platt scaling for probability calibration, and sigmoid mapping to 0–100 scores. Outputs `ForensicsFusedSignal[]` with probability, score, 5-bootstrap confidence bounds, and top contributors
- **Conformal Anomaly Detection** — conformal prediction p-values (not z-scores) using a 512-sample rolling calibration buffer per metric key stored in Redis. Dual scoring: value non-conformity (`|value − median| / IQR`) + timing non-conformity (log-interval deviation). Combined p-value is the minimum of the two; significance threshold α = 0.05. Outputs `ForensicsCalibratedAnomaly[]` with p_value, severity, calibration_count, and counterfactual levers
- **Causal Discovery** — MDL-based causal graph construction across **3 temporal horizons**: tactical (15-min buckets, 2 h lookback), operational (6 h buckets, 3 d lookback), and strategic (24 h buckets, 30 d lookback). Computes co-activation lift P(B|A)/P(B) and MDL causal score; emits up to 40 `ForensicsCausalEdge[]` per run with cause/effect signal types, causal_score, delay_ms, and conditional_lift
...
- **Milestone celebrations** — canvas-confetti bursts fire on positive environmental milestones (species recovery announcements, renewable energy record thresholds per 5%) detected in incoming data. Session-level deduplication ensures the same milestone is never celebrated twice. Warm nature-inspired color palette (greens, golds, blues). Respects `prefers-reduced-motion` — fully silent when the user has requested reduced animations
